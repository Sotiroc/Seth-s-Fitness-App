...
- [ ] Add sets
- [ ] Support weighted/bodyweight/cardio set entry
- [ ] Persist every change immediately
- [ ] Finish and cancel workout flows
...
**Order**
- Run Codex first on the data flow for workouts, sets, and persistence.
- Claude should build UI against the backend contract once the mutation/read paths are in place.
- Some polishing can happen in parallel after the backend shape is stable.
