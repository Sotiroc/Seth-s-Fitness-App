...
A step-by-step build plan, ordered so each phase gives you something usable before moving on. Tick items 
off as you go. Keep this doc in the repo root.

PROJECT_PHASE_0: DONE
PROJECT_PHASE_1: CURRENT
...
- [ Confirmed Look at Brand.md, Default mode should be Light Mode ] **Seed color & theme tokens:** jelly-bean `#289CB2` as seed. Decide default mode (dark or light) for first launch — recommend dark for a gym app.
- [ Confirmed - Lets use roboto] **Pick a font:** Inter or Manrope via `google_fonts` package. Clean, readable, free.
- [ Ai must just fill out this basic list, I will go over it later on ] **Decide default exercise list:** write out the 12–20 exercises to seed on first launch. Example starter set: Bench Press, Incline Dumbbell Press, Overhead Press, Pull-Up, Barbell Row, Lat Pulldown, Squat, Deadlift, Romanian Deadlift, Leg Press, Bicep Curl, Tricep Pushdown, Plank (bodyweight), Push-Up (bodyweight), Sit-Up (bodyweight), Treadmill (cardio), Stationary Bike (cardio).
...
- [ ] `flutter create workout_app` with org id you'd use for stores later (e.g., `com.yourname.workout`).
- [ ] Add dependencies: `drift`, `drift_flutter`, `path_provider`, `path`, `google_fonts`, `intl`, `uuid`. Dev deps: `drift_dev`, `build_runner`.
- [ ] Set up folder structure:
  ...
  lib/
    ...
    main.dart
  ```
- [✅] Build `AppTheme` class with `ColorScheme.fromSeed(seedColor: Color(0xFF289CB2))` for both light and dark. Wire `useMaterial3: true`.
- [✅] Add custom jelly-bean palette extension for cases where you want specific shades outside M3's generated scheme.
- [ ✅] Build a placeholder home screen with a bottom nav (Workouts, History, Exercises) so you can see the theme live.
- [ ✅ Looks good now ] Run on your physical device. Confirm the vibe. Adjust seed or brightness if needed.
