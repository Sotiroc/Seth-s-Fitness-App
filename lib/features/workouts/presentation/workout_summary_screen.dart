...
/// Shown after a workout is finished. Reads the finished workout by id via
/// [workoutDetailByIdProvider] and gives the user a one-glance summary.
class WorkoutSummaryScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final AsyncValue<WorkoutDetail> detailAsync = ref.watch(
      workoutDetailByIdProvider(workoutId),
    );

    return Scaffold(
      backgroundColor: palette.shade50,
      body: detailAsync.when(
        data: (detail) => _SummaryBody(detail: detail, palette: palette),
...
class _SummaryBody extends StatelessWidget {
  const _SummaryBody({required this.detail, required this.palette});
